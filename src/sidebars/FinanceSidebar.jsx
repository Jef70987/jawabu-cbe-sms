          ...
          <div className="flex items-center justify-between px-4 py-2">
            ...
            >
              ...
              <div className="flex flex-col leading-tight">
                ...
                <h2 className="text-white text-[10px] font-semibold leading-tight">Finance Portal</h2>
              </div>
            </button>
            <div className="text-right">
              <p className="text-white text-xs font-semibold">{user?.username || 'User'}</p>
              <p className="text-green-200 text-[10px] font-medium">{user?.role || 'Finance'}</p>
            </div>
          </div>
        </div>
      )}
        ...
        <div className="fixed bottom-0 left-0 right-0 bg-gradient-to-t from-green-800 to-green-700 shadow-lg z-50 lg:hidden">
          <div className="flex items-center h-12 px-2 overflow-x-auto">
            <div className="flex justify-around gap-1">
              {FinanceSidebarData.slice(0, 15).map((val, key) => (
                <button
                  key={key}
            ...
            <div className={`flex items-center ${isCollapsed ? 'flex-col' : 'space-x-4'}`}>
              ...
              {!isCollapsed && (
                <div className="flex flex-col">
                  ...
                  <h2 className="text-white text-xs font-semibold">FINANCE PORTAL</h2>
                  <p className="text-sm font-extrabold text-white truncate">{user?.username || 'User'} | {user?.role || 'Finance'}</p>
                </div>
              )}
            </div>
