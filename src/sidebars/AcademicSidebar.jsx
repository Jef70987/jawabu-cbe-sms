...
import { AcademicSidebarData } from '../SidebarData/AcademicSidebarData';
import { 
  ChevronDown, 
  ChevronRight, 
  LogOut, 
...
const AcademicSidebar = () => {
  ...
  useEffect(() => {
    const newOpenMenus = {};
    AcademicSidebarData.forEach((item, index) => {
  ...
  return (
    <>
      ...
      <button 
        ...
        className="lg:hidden fixed top-4 left-4 z-50 p-2.5 bg-gradient-to-r from-green-600 to-emerald-600 text-white rounded-xl shadow-lg hover:shadow-xl transition-all duration-300 transform hover:scale-105"
      ...
      `}>
        ...
        <div className="p-6 border-b border-gray-200 bg-gradient-to-r from-green-600 to-emerald-600">
          <div className="flex items-center space-x-3">
            ...
            <div>
              <h2 className="font-bold text-white text-lg">Academic Affairs</h2>
              <p className="text-xs text-green-100">Curriculum & Examinations</p>
            </div>
          </div>
        </div>

        {/* Profile Section */}
        <div className="p-5 border-b border-gray-200 bg-gradient-to-r from-green-50 to-emerald-50 relative overflow-hidden">
          <div className="absolute inset-0 bg-grid-pattern opacity-5"></div>
          <div className="flex items-center space-x-4 relative">
            <div className="relative">
              <div className="w-14 h-14 bg-gradient-to-br from-green-600 to-emerald-600 rounded-2xl flex items-center justify-center text-white font-semibold text-xl shadow-md transform transition-transform hover:scale-105">
                {user?.name?.split(' ').map(n => n[0]).join('') || 'AD'}
              </div>
              <div className="absolute -bottom-1 -right-1 w-4 h-4 bg-green-500 border-2 border-white rounded-full"></div>
            </div>
            <div className="flex-1">
              <p className="text-sm font-semibold text-gray-800">{user?.name || 'Academic Director'}</p>
              <p className="text-xs text-gray-600 flex items-center">
                <GraduationCap size={12} className="mr-1" /> Academic Affairs
              </p>
              <div className="flex items-center space-x-2 mt-1">
                <div className="flex items-center text-[10px] text-gray-500 bg-white/50 px-2 py-0.5 rounded-full">
                  <span>Last login: Today {new Date().toLocaleTimeString()}</span>
                </div>
              </div>
            </div>
          </div>
        ...
        <nav className="flex-1 overflow-y-auto py-4 scrollbar-thin scrollbar-thumb-gray-300 scrollbar-track-gray-100 hover:scrollbar-thumb-gray-400">
          <ul className="space-y-1 px-3">
            {AcademicSidebarData.map((item, index) => (
                ...
                {item.submenu ? (
                  <div className="mb-1">
                    <button
                      ...
                      } ${hoveredItem === index && !isActiveRoute(item.path) ? 'bg-green-50' : ''}`}
                    >
                      <div className="flex items-center space-x-3">
                            ...
                            <NavLink
                              ...
                              className={({ isActive }) =>
                                `block px-4 py-2.5 text-sm rounded-lg transition-all duration-200 relative group ${
                                  isActive
                                    ? 'text-green-600 font-medium bg-green-50 border-l-4 border-green-600'
                                    : 'text-gray-600 hover:bg-gray-100 hover:text-gray-900 hover:pl-6'
                                }`
                              }
                            >
                              <span className="relative z-10">{subItem.title}</span>
                              <span className="absolute inset-0 bg-gradient-to-r from-green-50 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-200 rounded-lg"></span>
                            </NavLink>
                          </li>
                        ))}
                      </ul>
        ...
        <div className="p-4 border-t border-gray-200 bg-gray-50 space-y-2">
          ...
          <div className="flex justify-around mb-3">
            <button 
              onClick={() => handleNavigation('/academic/notifications')}
              className="p-2 hover:bg-gray-200 rounded-lg transition-colors relative group"
              title="Notifications"
            >
              <Bell size={18} className="text-gray-600 group-hover:text-green-600" />
              <span className="absolute -top-1 -right-1 w-4 h-4 bg-red-500 text-white text-[10px] flex items-center justify-center rounded-full">6</span>
            </button>
            <button 
              onClick={() => handleNavigation('/academic/settings')}
              className="p-2 hover:bg-gray-200 rounded-lg transition-colors group"
              title="Settings"
            >
              <Settings size={18} className="text-gray-600 group-hover:text-green-600" />
            </button>
            <button 
              ...
              title="Help"
            >
              <HelpCircle size={18} className="text-gray-600 group-hover:text-green-600" />
            </button>
          </div>
          ...
          <p className="text-[10px] text-center text-gray-400 mt-2">Version 2.0.0 • Academic Portal</p>
        </div>
      </div>
