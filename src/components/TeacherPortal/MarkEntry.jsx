...
import {
  Save, X, Filter, Users, ClipboardList, Award,
  CheckCircle, AlertCircle, Loader2,
  Download, Printer, Edit, RefreshCw, Eye,
} from 'lucide-react';
...
const META = {
  EE1: { label: 'Exceptional', badge: 'bg-emerald-100 text-emerald-800 border-emerald-300' },
  EE2: { label: 'Very Good', badge: 'bg-emerald-100 text-emerald-700 border-emerald-200' },
  ME1: { label: 'Good', badge: 'bg-sky-100 text-sky-800 border-sky-300' },
  ME2: { label: 'Fair', badge: 'bg-sky-100 text-sky-700 border-sky-200' },
  AE1: { label: 'Needs Improvement', badge: 'bg-amber-100 text-amber-800 border-amber-300' },
  AE2: { label: 'Below Average', badge: 'bg-amber-100 text-amber-700 border-amber-200' },
  BE1: { label: 'Well Below Average', badge: 'bg-rose-100 text-rose-800 border-rose-300' },
...
const Toast = ({ type, message, onClose }) => {
  ...
  return (
    <div className={`fixed bottom-6 right-6 flex items-center gap-3 px-4 py-3 rounded-lg border shadow-lg ${styles[type]} z-50 max-w-sm`}>
      {icons[type]}
...
const MarksEntrySheet = () => {
  ...
  useEffect(() => {
    ...
    if (!saved) return;
    try {
      const { selectedTeacherId: tid, selClass: cls, selTerm: term,
              selAssessmentWindowId: windowId, selSubject: subject } = JSON.parse(saved);
  ...
  useEffect(() => {
    ...
    if (!selClass || !selTerm || !selAssessmentWindowId || !selSubject || !isAuthenticated) return;
    ...
    const restore = async () => {
      ...
      try {
        const strandRes = await apiFetch(`${API_BASE_URL}${EP.strandsForArea}?learning_area_id=${selSubject}`);
        const firstStrand = strandRes?.success && strandRes.data?.length ? strandRes.data[0] : null;
        if (!firstStrand) return;

        const subRes = await apiFetch(`${API_BASE_URL}${EP.substrandsForStrand}?strand_id=${firstStrand.id}`);
        const firstSub = subRes?.success && subRes.data?.length ? subRes.data[0] : null;
        if (!firstSub) return;

        const compRes = await apiFetch(`${API_BASE_URL}${EP.competenciesForSub}?substrand_id=${firstSub.id}`);
        const firstComp = compRes?.success && compRes.data?.length ? compRes.data[0].id : null;
        if (!firstComp) return;

        setLearningAreaId(selSubject);
        setLearningAreaName(subjects.find(s => String(s.id) === String(selSubject))?.area_name || '');
        setCompetencyId(firstComp);
        await _fetchStudentsWithWindow(selClass, selSubject, selAssessmentWindowId, firstComp);
      } catch { /* silently fail */ }
      finally { setLoadingStudents(false); }
    };
    restore();
  // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [selClass, selTerm, selAssessmentWindowId, selSubject, subjects, isAuthenticated, didRestore]);
  ...
  useEffect(() => {
    if (!didRestore) {
      setClasses([]); setSubjects([]); setStudents([]);
      setLearningAreaId(null); setLearningAreaName('');
      setAssessmentId(null); setCompetencyId(null);
    }
    if (!isAuthenticated) return;
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const data = await apiFetch(`${API_BASE_URL}${EP.assessmentWindows}?term=${selTerm}`);
        if (data?.success) {
          setAssessmentWindows(data.data || []);
        } else {
          showToast('error', 'Failed to load assessment windows for this term.');
        }
      } catch {
  ...
  const handleClassChange = async (e) => {
    ...
    setDidRestore(false);
    setSelClass(classId);
    setSelSubject(''); setSubjects([]); setStudents([]);
    setLearningAreaId(null); setLearningAreaName('');
    setAssessmentId(null); setCompetencyId(null);
    if (!classId) return;
  ...
  const handleSubjectChange = async (e) => {
    ...
    setStudents([]); setAssessmentId(null); setCompetencyId(null);
  ...
  const _loadStudentsForSubject = async (subjectId, windowId) => {
    setLoadingStudents(true);
    try {
      const strandRes = await apiFetch(`${API_BASE_URL}${EP.strandsForArea}?learning_area_id=${subjectId}`);
      const firstStrand = strandRes?.success && strandRes.data?.length ? strandRes.data[0] : null;
      if (!firstStrand) throw new Error('No strands found');

      const subRes = await apiFetch(`${API_BASE_URL}${EP.substrandsForStrand}?strand_id=${firstStrand.id}`);
      const firstSub = subRes?.success && subRes.data?.length ? subRes.data[0] : null;
      if (!firstSub) throw new Error('No substrands found');

      const compRes = await apiFetch(`${API_BASE_URL}${EP.competenciesForSub}?substrand_id=${firstSub.id}`);
      const firstComp = compRes?.success && compRes.data?.length ? compRes.data[0].id : null;
      if (!firstComp) { showToast('error', 'No competencies mapped yet.'); return; }

      setLearningAreaId(subjectId);
      setLearningAreaName(subjects.find(s => String(s.id) === String(subjectId))?.area_name || '');
      setCompetencyId(firstComp);
      await _fetchStudentsWithWindow(selClass, subjectId, windowId, firstComp);
    } catch {
      showToast('error', 'Failed to load students for this subject.');
    } finally { setLoadingStudents(false); }
  };

  const _fetchStudentsWithWindow = async (classId, laId, assessmentWindowId, compId) => {
    ...
    const mapped = d.data.map(s => {
      ...
      return { ...s, marks: { percentage: displayPercentage, comment: s.comment || '' }, savedRating: ratingCode };
    });
    setStudents(mapped);
  ...
  const refreshStudents = () => {
    if (selClass && learningAreaId && selAssessmentWindowId && competencyId) {
      setLoadingStudents(true);
      _fetchStudentsWithWindow(selClass, learningAreaId, selAssessmentWindowId, competencyId)
        .catch(() => {})
        .finally(() => setLoadingStudents(false));
    }
  };
  ...
  const handleSubmit = async (e) => {
    ...
    if (!assessmentId || !competencyId) {
      showToast('error', 'Please select class, term, assessment and subject first.');
      return;
    }
    setSaving(true);
    try {
      const marksPayload = students.map(s => {
        ...
        return { student_id: s.id, rating: ratingCode, comment: s.marks.comment || '' };
      }).filter(Boolean);
      ...
      const d = await apiFetch(`${API_BASE_URL}${EP.saveMarks}`, {
        method: 'POST',
        body: JSON.stringify({ assessment_id: assessmentId, competency_id: competencyId, marks: marksPayload }),
      });
      if (d?.success) {
        showToast('success', d.message || 'Marks saved successfully');
        refreshStudents();
      } else {
        showToast('error', d?.error || d?.errors?.join(', ') || 'Failed to save marks');
      }
    } catch (err) {
  ...
  const canSave = !!assessmentId && !!competencyId && students.length > 0 && !isLocked;

  // ─── Reusable Select ──────────────────────────────────────────────────────
  const Sel = ({ label, value, onChange, options, placeholder, disabled, busy }) => (
    <div>
      <label className="block text-[11px] font-bold text-gray-400 uppercase tracking-widest mb-1.5">{label}</label>
      <div className="relative">
        <select value={value} onChange={onChange} disabled={disabled || busy}
          ...
          {options.map(o => (
            <option key={o.id ?? o.value} value={o.id ?? o.value}>
              {o.label ?? o.class_name ?? o.name ?? o.area_name ?? o.term ?? o.full_name}
            </option>
          ))}
        </select>
  ...
  const StudentRow = ({ student, index, isLocked, onUpdate }) => {
    const [localPercentage, setLocalPercentage] = useState(student.marks.percentage || '');
    const [localComment, setLocalComment] = useState(student.marks.comment || '');

    const commitChanges = () => onUpdate(student.id, { percentage: localPercentage, comment: localComment });
    const handleKeyDown = (e) => { if (e.key === 'Enter') { e.preventDefault(); commitChanges(); } };
    const grade = getCbcGrade(localPercentage);

    return (
      <tr className="hover:bg-gray-50/70 transition-colors">
        <td className="px-5 py-3.5 text-gray-300 text-xs">{index + 1}</td>
        <td className="px-5 py-3.5 font-mono text-xs text-gray-500">{student.admission_no}</td>
        ...
        <td className="px-5 py-3.5">
          <input type="text" inputMode="numeric" value={localPercentage}
            onChange={e => setLocalPercentage(e.target.value)} onBlur={commitChanges} onKeyDown={handleKeyDown}
            disabled={isLocked} placeholder="0-100"
        ...
        <td className="px-5 py-3.5">
          {grade
            ? <div className={`inline-flex items-center px-3 py-1 text-xs font-bold rounded-lg border ${grade.cls}`}>{grade.code}</div>
            : <span className="text-gray-400 text-xs">—</span>}
        </td>
        <td className="px-5 py-3.5">
          <input type="text" value={localComment}
            onChange={e => setLocalComment(e.target.value)} onBlur={commitChanges} onKeyDown={handleKeyDown}
            disabled={isLocked} placeholder="Add a comment…" maxLength={200}
  ...
  const ViewTab = () => (
    <div className="bg-white border border-gray-200 rounded-xl overflow-hidden shadow-sm">
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="bg-gray-50 border-b border-gray-200">
              {['#', 'Adm. No.', 'Student Name', 'CBC Grade', 'Teacher Comment'].map(h => (
          ...
          <tbody className="divide-y divide-gray-100">
            {students.map((s, i) => {
              const ratingCode = s.savedRating || percentageToCbcCode(s.marks.percentage);
              return (
                <tr key={s.id} className="hover:bg-gray-50/70">
                  <td className="px-5 py-3.5 text-gray-300 text-xs">{i + 1}</td>
                  <td className="px-5 py-3.5 font-mono text-xs text-gray-500">{s.admission_no}</td>
                  <td className="px-5 py-3.5 font-medium text-gray-800">{s.full_name}</td>
                  <td className="px-5 py-3.5">
                    {ratingCode
                      ? <div className={`inline-flex items-center px-3 py-1 text-xs font-bold rounded-lg border ${META[ratingCode]?.badge || 'bg-gray-100'}`}>{ratingCode}</div>
                      : <span className="text-gray-400 text-xs">—</span>}
                  </td>
                  <td className="px-5 py-3.5 text-xs text-gray-600">{s.marks.comment || '—'}</td>
                </tr>
              );
            })}
          </tbody>
      ...
      <div className="bg-gray-50 border-t border-gray-100 px-5 py-3 text-xs text-gray-500">
        Total: <strong className="text-gray-700">{students.length}</strong> students
      </div>
    </div>
  );
    ...
    <form onSubmit={handleSubmit} className="space-y-5">
      <div className="bg-white border border-gray-200 rounded-xl shadow-sm">
        <div className="px-5 py-4 border-b border-gray-100 flex items-center gap-2">
          ...
          <span className="text-sm font-semibold text-gray-800">Select Assessment Criteria</span>
          {learningAreaName && (
        ...
        <div className="p-5 grid grid-cols-1 sm:grid-cols-5 gap-4">
          {isAdminOrRegistrar && (
            <Sel label="Teacher" value={selectedTeacherId} onChange={e => setSelectedTeacherId(e.target.value)}
              options={teachers} placeholder="Select teacher" />
          )}
          <Sel label="Class" value={selClass} onChange={handleClassChange}
            options={classes} placeholder="Select class" busy={loadingClasses} />
          <Sel label="Term" value={selTerm} onChange={handleTermChange}
            options={terms} placeholder="Select term" busy={loadingTerms} disabled={!selClass} />
          ...
          <Sel
            ...
            options={assessmentWindows.map(w => ({ id: w.id, label: w.assessment_type }))}
            placeholder={selTerm ? 'Select assessment' : 'Select term first'}
            disabled={!selTerm}
            busy={loadingAssessmentWindows}
          />
          <Sel label="Subject" value={selSubject} onChange={handleSubjectChange}
            options={subjects}
      ...
      {loadingStudents ? (
        ...
        <div className="bg-white border border-gray-200 rounded-xl overflow-hidden shadow-sm">
          {isLocked && (
            <div className="bg-amber-50 border-b border-amber-200 px-5 py-2.5 flex items-center gap-2 text-amber-700 text-sm">
              ...
              This assessment is <strong className="ml-1">locked</strong> and cannot be modified.
            </div>
          )}
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              <thead>
                <tr className="bg-gray-50 border-b border-gray-200">
                  {['#', 'Adm. No.', 'Student Name', 'Marks (%)', 'CBC Grade', 'Teacher Comment'].map(h => (
                    <th key={h} className="px-5 py-3.5 text-left text-[11px] font-bold text-gray-400 uppercase tracking-wider">{h}</th>
                  ))}
                </tr>
              </thead>
              <tbody className="divide-y divide-gray-100">
                {students.map((s, i) => (
                  <StudentRow key={s.id} student={s} index={i} isLocked={isLocked}
                    onUpdate={(studentId, { percentage, comment }) => {
                      setStudents(prev => prev.map(stu =>
                        stu.id === studentId ? { ...stu, marks: { ...stu.marks, percentage, comment } } : stu
                      ));
                    }} />
                ))}
              </tbody>
          ...
          <div className="bg-gray-50 border-t border-gray-100 px-5 py-3 flex items-center justify-between">
            <span className="text-xs text-gray-500">Total: <strong className="text-gray-700">{students.length}</strong></span>
            <button type="button" onClick={refreshStudents} className="flex items-center gap-1.5 text-xs text-gray-400 hover:text-blue-600 transition-colors">
        ...
        <div className="bg-white border border-gray-200 rounded-xl py-16 text-center">
          ...
          <p className="text-sm text-gray-400">No students found for the selected criteria.</p>
        </div>
      ) : null}
      ...
      <div className="max-w-screen-2xl mx-auto px-4 sm:px-6 lg:px-8 py-6 space-y-5">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-xl font-bold text-gray-900">Marks Entry Sheet</h1>
              <p className="text-xs text-gray-400 mt-0.5">CBC Competency-Based Assessment</p>
            </div>
          </div>
          <div className="flex items-center gap-1">
            <button className="p-2 rounded-lg text-gray-300 hover:text-blue-600 hover:bg-blue-50 transition-colors"><Download className="w-4 h-4" /></button>
            <button className="p-2 rounded-lg text-gray-300 hover:text-blue-600 hover:bg-blue-50 transition-colors"><Printer className="w-4 h-4" /></button>
          </div>
        </div>
        ...
        <div className="border-b border-gray-200">
          <nav className="flex gap-1 -mb-px">
            {[
              { id: 'entry', icon: <Edit className="w-4 h-4" />, label: 'Marks Entry' },
              { id: 'view', icon: <Eye className="w-4 h-4" />, label: 'View Saved Marks' },
            ].map(tab => (
